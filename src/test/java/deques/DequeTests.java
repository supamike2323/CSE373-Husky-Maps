...
//                System.out.println(deque.getClass().getSimpleName());
                ...
                // Populate the deque with initial elements
                for (int j = 0; j < size; j += 1) {
                    deque.addLast(j);
                }

                // Record the total runtimes accumulated across all trials for removeLast
                long totalRemoveTime = 0;

                for (int i = 0; i < NUM_TRIALS; i += 1) {
                    // Measure the time to remove one element from the end
                    long removeStart = System.nanoTime();
                    deque.addLast(size);
                    long removeTime = System.nanoTime() - removeStart;
                    // Add to total time
                    totalRemoveTime += removeTime;
                    // Add the removed element back to restore the deque's size
                    deque.removeLast();
                }

                // Output the average time taken for removeLast, rounded to the closest integer.
                System.out.printf("%.0f", totalRemoveTime / (double) NUM_TRIALS);
                System.out.println();
            }
        }
    }
